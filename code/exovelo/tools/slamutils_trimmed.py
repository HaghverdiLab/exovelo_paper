# requires: python-3.11 pandas polars scanpy anndata matplotlib sklearn scipy umap 
...
#
#def safeTensorNormalize(X, eps=1e-12):
#    X = X if isinstance(X, torch.Tensor) else torch.tensor(X)
#    return X / (eps + X.norm(dim=-1, keepdim=True) )

#def batchCosineScore(
#        X : np.ndarray,
#        V: np.ndarray,
#        eps : float = 1e-12,
#        ):
#    """
#    Estimate the "distance" between velocities V to the manifold defined by the points X.
#
#    parameters
#    ----------
#    `X` : `(n_obs, n_dims)` array type of observations
#    `V` : `(n_obs, n_dims)` array type of velocities
#    `epse` : epsilon minimal norm size
#
#    output:
#    -------
#    'scores' : `(n_obs,)` array of score values for each velocity in the range [0,1] where 0 means its orthogonal and 1
#    means its tangent to the manifold.
#
#    V and X are paired meaning v=V[i] is the velocity of x=X[i]
#    The calculate the score of (v,x) two nearest neighbors p,q of x are chosen.
#    the score is then `sqrt(cosine(p-x,v)**2 + cosine((q-x)', v)**2)` which technically gives the proportion of v that is
#    tangent to the plane defined by p,q,x. (q-x)' designates the component of (q-x) that is orthogonal to (p-x).
#    """
#    tree = BallTree(X, metric='l2')
#    indX = tree.query(X, k=3, return_distance=False)
#    P = X[indX[:,1]]
#    Q = X[indX[:,2]]
#    PX = safeTensorNormalize(torch.tensor(P-X))
#    QX = safeTensorNormalize(torch.tensor(Q-X))
#    QX = safeTensorNormalize(
#            QX - (QX * PX).sum(-1, keepdim=True) * PX
#            )
#    V = torch.tensor(V)
#    S = F.cosine_similarity(V, PX, dim=-1)**2 + F.cosine_similarity(V,QX,dim=-1)**2
#    return S.sqrt().numpy()
...
def tf_idf(
    bow : np.ndarray,
    #bow : Union[torch.Tensor, np.ndarray],
